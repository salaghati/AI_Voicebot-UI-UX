  ...
  return (
    <div className="space-y-4">
      <PageHeader
        ...
        description="Quản lý tuyến inbound, queue/extension và trạng thái."
        actions={
          <Link href="/bot-engine/inbound/create">
            <Button className="gap-2"><Plus className="h-4 w-4" /> Tạo Inbound</Button>
          </Link>
        }
      />
          ...
          return (
            <Card key={item.id} className="relative flex min-h-[280px] flex-col justify-between p-4">
              <div>
                ...
                <Link href={`/bot-engine/inbound/${item.id}`} className="mt-3 block text-[28px] leading-tight font-semibold text-[var(--text-main)] hover:text-[var(--accent)]">
                  {item.name}
                </Link>
              ...
              {menuId === item.id ? (
                <div className="absolute right-4 top-10 z-20 w-48 rounded-xl border border-[var(--line)] bg-white p-1 shadow-xl">
                  <Link href={`/bot-engine/inbound/${item.id}`} className="block rounded-lg px-3 py-2 text-sm hover:bg-[var(--surface-2)]">
                    Xem chi tiết
                  </Link>
                  <button
                    type="button"
